...
Before routing a request, CareerLens checks that each provider's required
environment variables are present and skips unconfigured providers without
exposing credential values. Configured providers receive one automatic retry
for transient connection, timeout, quota, server, or malformed-response
failures. Authentication failures are not retried, and the router continues to
